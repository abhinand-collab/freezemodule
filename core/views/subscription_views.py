...
def subscription_list_view(request):
    if request.method == 'POST' and request.headers.get('x-action') == 'fetch':
        ...
        paginator = Paginator(subscription_plans_queryset, 10)
        page_obj = paginator.get_page(page_number)
        page_range = list(paginator.get_elided_page_range(page_obj.number, on_each_side=2, on_ends=1))
        
        serializer = SubscriptionPlanSerializer(page_obj.object_list, many=True)
        return JsonResponse({
            'status': 'success', 
    ...
    if request.method == 'POST':
        ...
        if serializer.is_valid():
            ...
            if request.headers.get('x-requested-with') == 'XMLHttpRequest':
                return JsonResponse({'status': 'success', 'message': 'Subscription plan added successfully.'})
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest' and request.method == 'GET':
        serializer = SubscriptionPlanSerializer(subscription_plans_queryset, many=True)
